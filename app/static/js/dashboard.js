...
async function refreshHostsList() {
    ...
    try {
        ...
        if (hosts.length === 0) {
            createEl('div', ['p-4', 'text-center', 'text-muted'], 'No hosts yet. Add one in the Configuration panel.', hostsContainer);
            return;
        }
        hosts.forEach(renderDashboardRow);
    } catch (err) {
        console.error(err);
        createEl('div', ['alert', 'alert-danger'], 'Error loading hosts from the API.', hostsContainer);
    }
}
...
function renderDashboardRow(host) {
    ...
    createEl('div', ['text-muted', 'small', 'text-center', 'fst-italic'], 'Click Status...', colStatus);
    ...
    const logsBtn = createEl('button', ['btn', 'btn-outline-dark'], 'Logs', btnGroup);
    logsBtn.title = "Collect and analyze logs (SIEM)";
...
async function handleCheckStatusFancy(host, container, btn) {
    ...
    createEl('div', ['text-muted', 'small', 'text-center'], 'Connecting...', container);
    ...
    try {
        ...
        clearContainer(container);
        createEl('div', ['text-danger', 'small', 'fw-bold', 'text-center'], 'Error', container);
        btn.innerHTML = 'Retry';
    } finally {
        btn.disabled = false;
    }
...
async function handleFetchLogs(host, btn) {
    ...
    try {
        ...
        const alertCount = result.alerts || 0;
        if (alertCount > 0) {
            btn.innerHTML = '⚠️ ' + alertCount;
        ...
        alert("Error collecting logs: " + err.message);
        btn.textContent = originalText;
        btn.disabled = false;
    }
}
...
async function refreshAlertsTable() {
    ...
    try {
        ...
        if (alerts.length === 0) {
            ...
            const cell = createEl('td', ['text-center', 'text-muted', 'py-3'], 'No alerts yet.', row);
            cell.colSpan = 6;
            return;
        }
        ...
        alerts.forEach(alert => {
            ...
            const utcDate = new Date(alert.timestamp.replace(" ", "T"));
